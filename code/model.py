...
class BERT_ENCODER(nn.Module):
    def __init__(self, ninput, drop_prob=0.5):
    ...
    def forward(self, captions, mask=None):
        outputs = self.bert(captions, attention_mask=mask)
        words_emb = self.drop(outputs.last_hidden_state)
        # words_emb = words_emb.transpose(1, 2)  
        words_emb = self.word_linear(words_emb).transpose(1, 2)  
        
        # Dropout and linear transformation for pooled (sentence) embeddings
        sent_emb = self.drop(outputs.pooler_output)
        sent_emb = self.sent_linear(sent_emb)  
